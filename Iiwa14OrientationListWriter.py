...
with open('iiwa141762967540628.9177.csv', newline='') as csvfile:
    ...
    for row in tqdm(range(1, len(csvreader))):
        ...
        boolean_list.append(int(csvreader[row][4]))
        ...
        if i == 500:
            boolean_matrix.append(boolean_list)
            boolean_list = []
            i = 0
...
index_count = 0
boolean_solutions_tensor = np.zeros((len(x), len(z), len(a))).tolist()
for inner in range(len(outer_formatter)):
    for z_finder in outer_formatter[inner]:
        z_index = z.index(z_finder)
        boolean_solutions_tensor[inner][z_index] = boolean_matrix[index_count]
        index_count += 1

for matrix in range(len(boolean_solutions_tensor)):
    for vector in range(len(boolean_solutions_tensor[matrix])):
        if all([v == 0 for v in boolean_solutions_tensor[matrix][vector]]):
            boolean_solutions_tensor[matrix][vector] = [0]*500

    '''boolean_solutions_tensor = []
    boolean_solutions_tensor_middle = []
    boolean_solutions_tensor_inner = []
    past_x = tuple(map(float, csvreader[1][0].replace("(", "").replace(")", "").split(', ')))[0]
    past_z = tuple(map(float, csvreader[1][0].replace("(", "").replace(")", "").split(', ')))[2]
    index_count = 0
    list_num = 0
    ...
    for row in tqdm(range(1, len(csvreader))):
        current_x = tuple(map(float, csvreader[row][0].replace("(", "").replace(")", "").split(', ')))[0]
        current_z = tuple(map(float, csvreader[row][0].replace("(", "").replace(")", "").split(', ')))[2]
        current_index = z.index(current_z)
        while True:
            if current_index != index_count:
                boolean_solutions_tensor_middle.append([])
                index_count += 1
            else:
                if not current_z == past_z:
                    boolean_solutions_tensor_middle.append(boolean_matrix[list_num])
                    list_num += 1
                    if not current_x == past_x:
                        boolean_solutions_tensor.append(boolean_solutions_tensor_middle)
                        boolean_solutions_tensor_middle = []
                        index_count = 0
                past_x = current_x
                past_z = current_z
...
if len(boolean_solutions_tensor) != len(x):
    print(len(boolean_solutions_tensor))
    print('outer incorrect')
for outer in boolean_solutions_tensor:
    if len(outer) != len(z):
        print(len(outer))
...
with open("boolean solution tensor.csv", "w", newline="") as f:
    w = csv.writer(f)
    for row in boolean_solutions_tensor:
        w.writerow(row)
